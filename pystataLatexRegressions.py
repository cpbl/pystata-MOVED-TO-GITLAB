...
"""
    ...
    def __init__(self,filename,margins='default',colour=True,modelVersion=None,regressionVersion=None,compactPreview=True,codebook=None,mainSurvey=None,mainDataFile=None,recreateCodebook=None,substitutions=None,texNameSuffix=None):
        ...
        self.txtNamesUsed=[]
        self.updateSettings(codebook=codebook,mainSurvey=mainSurvey,mainDataFile=mainDataFile,recreateCodebook=recreateCodebook)
            
        if filename.endswith('.tex'):
            filename=filename[:-5]
        self.fname=filename+('-'+self.modelVersion)*(not not self.modelVersion)+('-'+self.regressionVersion)*(not not self.regressionVersion)
        ...
        #lfile.write(thead)
        #lfile.close()
        self.lfileTeXwrapper=[thead,'\n'+r'\clearpage\newpage\end{landscape}'+' End\n\\end{document}\n']
        #self.lfileTeX_original=self.lfileTeXbody+'' # Maybe obselete now, since I have body separate
        
    def updateSettings(self,codebook=None,mainSurvey=None,mainDataFile=None,recreateCodebook=None):
    ...
    ###########################################################################################
    ###
    def appendRegressionTable(self,models, suppressSE=False,substitutions=None,transposed=None, tableFilePath=None,tableFormat=None,sourceLogfile=None):# tableCaption=None, tableComments=None,modelTeXformat=None,extrarows,
        ###
        #######################################################################################
        """
    ...
    ###########################################################################################
    ###
    def old_forPairedRows_appendRegressionTable(self,colnames,colnums,coefrows,extrarows,greycols=None,suppressSE=False,substitutions=None,modelTeXformat=None,transposed=None,
                       tableFilePath=None, tableCaption=None, tableComments=None,
                       landscape=False,rowModelNames=None,hideRows=None): # landscape is deprecated: it's chosen automatically.
        ###
        #######################################################################################
        # Add this table as in include in a master LaTeX file that includes all the tables...
        # Do landscape, if desired. Decide this separately for each table.
...
"""
        if isinstance(transposed,str) and transposed=='both':
            self.old_forPairedRows_appendRegressionTable(colnames,colnums,coefrows,extrarows,suppressSE=suppressSE,substitutions=substitutions,modelTeXformat=modelTeXformat,
                       tableFilePath=tableFilePath, tableCaption=tableCaption, tableComments=tableComments,
                       landscape=landscape,transposed=True,rowModelNames=rowModelNames,hideRows=hideRows)
            self.old_forPairedRows_appendRegressionTable(colnames,colnums,coefrows,extrarows,suppressSE=suppressSE,substitutions=substitutions,modelTeXformat=modelTeXformat,
                       tableFilePath=tableFilePath, tableCaption=tableCaption, tableComments=tableComments,
                       greycols=greycols,landscape=landscape,transposed=False,rowModelNames=rowModelNames,hideRows=hideRows)
            return
        if tableFilePath==None:
            tableFilePath=defaults['paths']['tex']+'tmpMissingTableName.tex'
        if tableCaption==None:
        ...
        includedTex,wrapperTex,transposedChoice=old_uses_pairedRows_composeLaTeXtable(colnames,colnums,coefrows,extrarows,suppressSE=suppressSE,
                        substitutions=substitutions,modelTeXformat=modelTeXformat, caption=tableCaption,greycols=greycols,comments=tableComments,transposed=transposed,rowModelNames=rowModelNames,hideRows=hideRows,landscape=landscape)
...
##             self.append('\n'.join([ texOpening, includedTex , texClosing]))
    ...
    ###########################################################################################
    ###
    def toDict(self,line,depvar=None,regoptions=None,method=None,defaultValues=None): # Used to be called regDict
        ###
        #######################################################################################
        ...
        for optparam,kk in [[depvar,'depvar'],[regoptions,'regoptions'],[method,'method']]:
            if optparam:
...
##             dd['method']=method
    ...
    ###########################################################################################
    ###
    def withCellDummies(self,lines,cellVariables,cellName=None,nCounts=None,clusterCells=True,dropvars=None,minSampleSize=300,defaultModel=None):#depvar=None,regoptions=None):
        ###
        #######################################################################################
...
OCt 2008: Eliminated "depvar" and "regoptions" in favour of defaultModel, which can have the former two fields.
        ...
        if isinstance(lines,dict):
            ...
            lines=[lines]
        elif   isinstance(lines,list) and all([isinstance(onee,dict) or (isinstance(onee,list) and isinstance(onee[0],str)) for onee in lines]):
            debugprint('Found list of nothing but ',len(lines),' models (in dict or old format); so proceed with standard loop that will treat them as one group')
            for mm in lines:
                mm=self.toDict(mm)
                #mm=self.convertMtoDict(mm)

        else:
            debugprint('so must have a list of ',len(lines),' lists/modelslists, or else the models are not in dict format?')
            for lineOrGroup in lines:
                debugprint('For a linegroup ',lineOrGroup,' generated ',self.withCellDummies(lineOrGroup,cellVariables,cellName=cellName,nCounts=nCounts,clusterCells=clusterCells,dropvars=dropvars,minSampleSize=minSampleSize,defaultModel=defaultModel))#depvar=depvar,regoptions=regoptions))
                modelsout+=self.withCellDummies(lineOrGroup,cellVariables,cellName=cellName,nCounts=nCounts,clusterCells=clusterCells,dropvars=dropvars,minSampleSize=minSampleSize,defaultModel=defaultModel)#,depvar=depvar,regoptions=regoptions)
        ...
        for model in deepcopy(lines):
            ...
            if not dropvars==None:
                for dv in uniqueInOrder([dvv for dvv in dropvars.split(' ') if dvv and dvv in useRegressors]):
                    useRegressors.remove(dv)#=useRegressors.replace(' '+dv+' ','  ')

            # Also, just for the dummy selection, remove any other variables which we do not think exist for any of the chosen surveys:
            expectedExistRegressors=['1']+ [rr for rr in useRegressors if inanyCodebook(rr,surveys)]
            droppedList=set(expectedExistRegressors)-set(useRegressors)-set(['1'])
            if droppedList:
            ...
            stataBeforeOut+='\n bysort '+cellVariables+': egen ttt_'+cellName+'=count(dummyOne)  if '+parts[1]+' & '+dsurveys+ ' & ' + ' & '.join(['%s<.'% rr for rr in useRegressors])
            # Also ensure (if depvar is known) that there is at least some variation in depvar within each cell
            if 'depvar' in model:
                stataBeforeOut+='\n bysort '+cellVariables+': egen tttvvv_'+cellName+'=sd('+model['depvar']+')  if '+parts[1]+' & '+dsurveys+ ' & ' + ' & '.join(['%s<.'% rr for rr in useRegressors])
            else:
            ...
            quietly: tab dcelltmp if ttt_"""+cellName+""" >= """+str(nCounts)+""" & tttvvv_"""+cellName+""">0 & ttt_"""+cellName+""" <. &  """+parts[1] + """, gen(ddd_"""+cellName+""")
            drop dcelltmp
            """
            ...
            if clusterCells:
                ...
                if 'cluster(' in model['regoptions']:
                    ...
                    if isinstance(model.get('flags',''),dict):
                        model['flags']['clustering']='{\smaller \smaller %s}'%cellName
                    else:
                        model['flags']+=[['clustering',r'{\smaller \smaller %s}'%cellName]]
    ...
    ###########################################################################################
    ###
    def withCRdummies(self,models,CRs,each=False,nCounts=None,clusterCRs=True,minSampleSize=800,defaultModel=None,manualDrops=None): # aka "byCR"
        ###
        #######################################################################################
...
manualDrops = ??
    ...
    """
        ...
        if isinstance(models,dict):
            debugprint('Found dict')
        elif   isinstance(models,list) and all([isinstance(onee,dict) for onee in models]):
        ...
        if not isinstance(models,dict) and not isinstance(models[0],dict):
            ...
            for modelOrGroup in models:
                if isinstance(modelOrGroup,dict):# or not isinstance(modelOrGroup[0],list):
                    ...
                    pass
                modelsout+=self.withCRdummies(modelOrGroup,CRs,each=each,nCounts=nCounts,clusterCRs=clusterCRs,minSampleSize=minSampleSize,defaultModel=defaultModel,manualDrops=manualDrops)
            return(modelsout)
        """
        ...
        higherLevels={'CT':['CT','A15','CSD','HR','CMA','PR'],
                      'CSD':['CSD','HR','CMA','PR'],
                      'HR':['HR','CMA','PR'],
                      'CMA':['CMA','PR'],
                      'PR':['PR'],
        ...
        assert all([isinstance(model,dict) for model in models]) # Otherwise out of date / impossible
        ...
        topCoefficient=dict([[CRs[iCR],(CRs[1:]+[''])[iCR]] for iCR in range(len(CRs))])
...
##                 continue
            ...
            for model in deepcopy(models):
                ...
                model['CRgroup']={'id':'CR%03d'%globalGroupCounter,'fixedeffects':aCR,'takeCoef':topCoefficient[aCR],'addend':len(models)>1}#,'isaddend':''}}
                ...
                if aCR=='': # Make one copy with no dummies
                    ...
                    if aCR in crlist:
                        ...
                        acrsuffix=''
                    modelC=self.withCellDummies([model],aCR+acrsuffix,cellName=aCR,nCounts=nCounts,clusterCells=clusterCRs,minSampleSize=minSampleSize,defaultModel=defaultModel)[0]
    ...
    ###########################################################################################
    ###
    def removeUnavailableVariables(self,indepvars,survey,forceKeepVars=None,forceDropVars=None):
        ...
        ********* How does this relate to the fancier removeUnavailableVars, which is wave and country-dependent? I guess this just looks for vars that are completely unknown.
        ...
        if 1: # I think I must have lost a whole bunch of code right here, since I got an indenting error::
            ...
            unavailable=[]#[['unemployed',['GSS17','GSS19']],['mastery',['GSS19']],['lnhouseValue',['ESC2','GSS17']],['mortgagePayment',['ESC2','GSS17']],]
            removed=[]
            ...
            for var in [vv for vv in indepvarsReg.split(' ') if vv]:
                if var in forceDropVars or (not inanyCodebook(var,survey) and not inanyCodebook(var.replace('lnR','').replace('ln',''),survey) and not isGeneralVar(var) and not var in forceKeepVars):
    ...
    ###########################################################################################
    ###
    def bysurvey(self,surveys,model,includePooled=False,eliminateMissing=True,forceKeepVars=None):
        #                 withCRdummies=CRs,withCRdummiesCounts=None):
        ###
        #######################################################################################
        """
        ...
        if isinstance(model,dict):
            ...
            return(self.bysurvey(surveys,self.toDict(model),includePooled=includePooled,eliminateMissing=eliminateMissing,forceKeepVars=forceKeepVars))
        elif isinstance(model,list) and  isinstance(model[0],dict): # added late July 2008: dangerous.. this has been working for a while. Why now not?
            " This is a list of dicts. Convert each one to a set over surveys."
            fafa=self.bysurvey(surveys,model[0],includePooled=includePooled,eliminateMissing=eliminateMissing,forceKeepVars=forceKeepVars)
            return([self.bysurvey(surveys,oneDict,includePooled=includePooled,eliminateMissing=eliminateMissing,forceKeepVars=forceKeepVars)[0] for oneDict in model])
...
##             return(modelsout)
        ...
        for surv in surveys:
            ...
            if 'depvar' in model:
                if not inanyCodebook(model['depvar'].replace('lnR','').replace('ln',''),surv)  and not isGeneralVar(model['depvar']) and not model['depvar'] in forceKeepVars:
                    debugprint( 'Dropping this regression altogether! %s is not in %s'%(model['depvar'],surv))
                    continue
            ...
            if eliminateMissing:
                mm,removeda=self.removeUnavailableVariables(mm,surv,forceKeepVars=forceKeepVars,forceDropVars=forceDropVars)
                removed+=removeda
            else:
                removed=[]
            ...
            if 'flags' not in newrow:
                newrow['flags']=[]
            newrow['flags']+=[['survey',surv]]# This "surv" will be translated later into a shortname version and in small font, but do not do it here, since this model liine may be parsed further.
        ...
        if includePooled and len(surveys)>1:
            ...
            if ' if ' in model['model']:
                mma=model['model'].replace(' if ',' if (%s) & '%conditionAny)
            else:
                mma=model['model']+' if (%s) '%conditionAny
            if eliminateMissing:
                for surv in surveys:
                    mma,removeda=self.removeUnavailableVariables(mma,surv,forceKeepVars=forceKeepVars)
                    removed+=removeda
            else:
                removed=[]
    ...
    def _OBSELETE_DELETEME_readRegressionFile(self,tableFileName,useOUTREG2=False):
        ...
        if  not  os.path.exists(tableFilePathNoSuffix+'.txt'):
            ...
            return(r'\newpage{ Missing %s... Rerun Stata}\newpage'%str2latex(tableFileName))
        ...
        if not useOUTREG2 and len(open(tableFilePathNoSuffix+'.txt','rt').readlines())>97:
            print " Cannot use est2tex with more than 100 output lines. Stupid junk..."
            assert 0

        # Note the date that this regression was done.
        import time,os
        regressionDate=time.ctime(os.path.getmtime(tableFilePathNoSuffix+'.txt'))
        ...
        infileUsedOutreg2=open(tableFilePathNoSuffix+'.txt','rt').readlines()[1][0:9]=='VARIABLES'
        if not  useOUTREG2==infileUsedOutreg2:
            print ('************** OUTREG2 CHOICE DOES NOT MATCH!! SKIPPING THIS TABLE!')
            print tableFileName+': outreg2='+str(useOUTREG2)
            return(None)#return(outs)
        
        
        if not useOUTREG2:
            trows=[line.strip('\n\r').split('\t')[1:] for line in open(tableFilePathNoSuffix+'.txt','rt').readlines()][1:] #Drop first row; drop first column of ech row
...
"""
            ...
            trows=[line.strip('\n\r').split('\t')[0:] for line in open(tableFilePathNoSuffix+'.txt','rt').readlines()][3:-1] #Drop first three rows and final ; drop first column of ech row
...
"""
        ...
        for aline in lines: #TOdo2015: Integrate use of parseStataComments(txt) here:
            ...
            if method in ['svy:reg','svy:regress','reg','areg','regress','ologit','oprobit','logit','probit','xtreg','ivregress 2sls','ivreg 2sls']:
                ...
                toaddmodel.update({'model':model,'depvar':depvar,'method':method,'regoptions':regoptions,'code':{before:precode,'after':'',}})
    ...
    def updateToDefaultModel(self,models,defaultModel):
        ...
        if 1:
            for amodel in models:
                for field in defaultModel:
                    assert(field not in amodel or field=='flags' or amodel[field]==defaultModel[field]) # I am attempting to overwrite something. This is dangerous. Except that I allow extra "flags" to be specified this way.
        ...
        return() # I do not think it is necessary to return anything... The above should be written not to overwrite pointers.
    ...
    ###########################################################################################
    ###
    def suestTests(self,twomodels,skipStata=False,tablename=None,modelSuffix=None): 
        ...
        snames=[twomodels[0]['stataStoredName'],twomodels[1]['stataStoredName']]
        m1,m2=twomodels
        assert snames[0]
        assert snames[1]
        ...
        vlists=[[mm for mm in twomodels[0]['model'].split(' if ')[0].split(' ') if mm not in ['cons']],    [mm for mm in twomodels[1]['model'].split(' if ')[0].split(' ') if mm not in ['cons']]]
        ...
        *END SUEST TESTS TWOMODELS
        """)%{'sn0':snames[0],'sn1':snames[1],'vvs':' '.join(regressors),'sfx1':modelSuffix[0],'sfx2':modelSuffix[1]}

        commonFlags=dict([[a,b] for a,b in m1.get('flags',{}).items()   if a in m2.get('flags',{}) and b==m2['flags'][a] ])

        assert not any(['ncome' in mm['depvar'] for mm in twomodels])
        print '  Creating SUEST TESTs for models %s  and %s '%(m1['name'],m2['name'])

        return(dict(special='suestTests',name=r'$p$(equal)',flags=commonFlags,code={'after':statacode},method='suest',model=' '.join(regressors),depvar=depvars[0]))
    ...
    def regTable(self,tablename,models,method=None,depvar=None,regoptions=None,variableOrder=None,showonlyvars=None,hidevars=None,forceShowVars=None,extralines=None,comments='',substitutions=None,options='',attributes=None,landscape=False,transposed=None,combineRows=None,suppressSE=False, produceOnly=None,extraTexFileSuffix=None,doPcorr=False,stopForErrors=True,crcoefsVars=None,skipStata=False,skipLaTeX=False,hidePSumTest=False,defaultModel=None,hideModelNumbers=False,assignSaveCoefficientsPrefix=None,hideModels=None,showModels=None,hideModelNames=False,renumberModels=True,showFailedRegressions=False,multirowLabels=False,betas=False,followupFcn=None,followupArgs=None,showCompDiff=None,returnModels=False,postPlotFcn=None,postPlotArgs=None,autoCreateVars=True,captureNoObservations=None,skipReadingResults=False): # Do a set of regressions; output results to .tex and .txt files 
        ...
        if stopForErrors==False:
            print ('******************* WARNING!!!!!!! WHY WOULD YOU USE STOPFORERRORS=FALSE??? tHIS IS SUPPRESSING ALL OUTPUT, INCLUDING ERRORS AND WARNINGS, FROM REGRESSIONS!*********** Use the autoexcludevars flag in the model struct instead!!! April2010: No, use replaceFailsWithDummy. No, use the object-level or table-level captureNoObservations')# Well, it should be called dummiesForEmptyRegressions, which is better.: capture reg adn then if _rc==2000, do dummy. (or 2001, insufficient obs)
            1/0
...
?
        ...
        def parseFlags(flags):
            ...
            elif isinstance(flags,list): # model[2] must be a list if not a str.
        ...
        assert not (variableOrder and showonlyvars) # Only one should be specified
        # Could I not just set variableorder to showonly vars here, too? Trying that, Aug 2009:
        if showonlyvars:
            variableOrder=showonlyvars

        assert extraTexFileSuffix or  (not  skipStata==True) or produceOnly # Otherwise this output .tex will overwrite the main one (though it may not be used). If you use "skipStata", provide the suffix to differentiate the LaTeX output.
        ...
        possibleIncomeVars=(possibleIncomeVars + possibleIncomeVars.replace('_','-')).split(' ')
...
""") # Don't have figures etc mixed between sections of relevant tables.
        ...
        if self.compactPreview and not skipLaTeX:
            self.append('\n%'+[r'\section',r'\subsection'][int(skipStata==True)]+'{%s (%s) ~[%s]}'%(tablename.replace('_','~'),extraTexFileSuffix.replace('_','~'),produceOnly)+'\n')
        elif not skipLaTeX:
            self.append('\n'+[r'\section',r'\subsection'][int(skipStata==True)]+'{%s (%s) ~[%s]}'%(tablename.replace('_','~'),extraTexFileSuffix.replace('_','~'),produceOnly))
        ...
        def parseModelSeparators(mods):
            iSeparators=[ii for ii in range(len(mods)) if isinstance(mods[ii],str) and mods[ii]=='|']
            if 0 in iSeparators:
        ...
        for row in models:
            if isinstance(row,list) and isinstance(row[0],dict): # If this "model" is really a group of models
                ...
                if any(['meanGroupName' in model for model in row]):
                    assert all(['meanGroupName' and model['meanGroupName'].__class__ in [str,unicode]  for model in row])
                ...
                if len(row)>1: # If there is more than one model in this group of models
                    sumGroup+=1 # Label for new group of models to sum
                    for mm in row:
                        assert isinstance(mm,dict)
                        if not 'meanGroupName' in mm:
                           mm['meanGroupName']=str(sumGroup) # Hm.. I am using meanGroupName for both numbers (auto) and string (named) format. 
                    print( ' Found group "%s" of %d regressions whose coefficients I should aggregate...'%(mm.get('meanGroupName',str(sumGroup)),len(row)))
        ...
        for im,mm in enumerate(models):
            for imm in range(im+1,len(models)):
                ...
                if models[im] is models[imm]:
                    print ' ** Warning!!!  Your list of models includes duplicate pointers (%dth=%dth). Is that intentional?... If so, you might want to use deepcopy. '%(im,imm)
                    1/0
        ...
        for irow in range(len(models))[::-1]:
            ...
            if 0 and isinstance(models[irow],str): #'format' not in models[irow]:
        ...
        if 0: # I don't think I am using these separators yet. Add that feature later.
            ...
            for irow in range(len(models))[1:][::-1]:
                ...
                if (isinstance(models[irow],str) and models[irow]=='|') or irow in [cc[0] for cc in combineColumns]:
                    models[irow-1][3]='c|'
        ...
        if not defaultModel:
            defaultModel={}
        defaultArgs=[['depvar',depvar],['method',method],['regoptions',regoptions]]
        for defarg,argval in defaultArgs:
            assert not (argval and defarg in defaultModel) #Can't specify value through both forms
            if argval:
...
#        allCovariates= ' '.join( uniqueInOrder(re.split(' ',' '.join(allspecs)))  )
        ...
        * CPBL BEGIN TABLE:%s: AT %s
        ...
        """%(tableLogNameWithDate,tablename,time.strftime('%Y_%m_%d_%H%M%S'))
...
di " CAUTION!!!!!!!! Replaced `var' values in MODEL:%s_%d:"""%(tablename,im+1)+""", since they were all missing!"
}
}
"""
...
if _rc~=0 {
di " CAUTION!!!!!!!! `var'  DID NOT EXIST in MODEL:%s_%d:"""%(tablename,im+1)+"""!"
capture gen `var'=0
}
}
"""
    ...
    """%(' '.join(['z_%s'%vv  for vv in zscoreVars.split(' ') if vv and not '*' in vv]))
    # """ " ` "
                if combineRows==None:
                    combineRows=[]
                combineRows=[[vv,'z_'+vv] for vv in zscoreVars.split(' ') if vv and not '*' in vv] +combineRows

                debugprint( 'regTable: USING BETA MODE FOR THIS REGRESSION: dropping all z_* variables')
                ...
                model['code']['after']+="""
                 ...
                 """
                if 0:
                    assert 'dCountry*' not in model['code']['before'] # Why? March2013: This was not commented. So I don't understand it and I'm removing it for now.
...
""" # No loonger, above, need to repeat regression, since used capture NOISILY
            ...
            if returnModels and not substitutions==self.substitutions: # Mod'd 2010 Sept; will need to update accounting code to use self.subs...?
                model['substitutions']=substitutions # Will be needed for formatting output of accounting analysis
 ...
 mean """+ model['depvar']+' '+RHSvarsNoWildcards + 0*thisRegVarsUsedNoWildcard + ' [pw=weight] if cssaSample & ('+ifs+"""),
                *END MEAN LIST MULTI

                *BEGIN SUM LIST MULTI
 sum """+  model['depvar']+' '+RHSvarsNoWildcards + 0*thisRegVarsUsedNoWildcard + ' [w=weight] if cssaSample & ('+ifs+"""), separator(0) nowrap
                *END SUM LIST MULTI

                """ for ifs in model['getSubSampleSums'] ]) ##+model['code']['after']
            ...
            if assignSaveCoefficientsPrefix:
                ...
                #if len(pieces)==1:
                #    pieces+=[' 1 ']
                for var in [vv for vv in pieces[0].split(' ') if vv and '*' not in vv]:
                    outs+='\nreplace %s%s=_b[%s] if %s\n'%(assignSaveCoefficientsPrefix,var,'z_'*(doBetaMode)+var,pieces[1])
                    outs+='\nreplace s%s%s=_se[%s] if %s\n'%(assignSaveCoefficientsPrefix[1:],var,'z_'*(doBetaMode)+var,pieces[1]) # old jujnk:'beta' in modelregoptions
    ...
    estimates table , varwidth(49) style(oneline) b se p stats(r2  r2_a r2_p N  N_clust ll r2_o) """+('drop('+dropIndicators+')' if dropIndicators else '')+"""
    * ereturn list
    """%()
...
"""%(model['depvar']+' '+thisRegRHSvars)#(model['depvar']+' '+RHSvarsNoWildcards)#thisRegRHSvars#thisRegVarsUsed
...
copy "%s" "%s", replace
        ...
        for mmm in models:
            if 'shortnames' in defaults:
                shortname=defaults['shortnames']
                if 'survey' in mmm['textralines'] and mmm['textralines']['survey'] in shortname:
                    mmm['textralines']['survey']=mmm['textralines']['survey'].replace(mmm['textralines']['survey'],shortname[mmm['textralines']['survey']])
            if 'survey' in mmm['textralines']:
                mmm['textralines']['survey']=r'{\smaller\smaller %s}'%mmm['textralines']['survey']
        ...
        if skipReadingResults:
            if not all(['eststats' in mm for mm in models]):
                ...
                self.append(r'\newpage{ Missing %s... Rerun Stata}\newpage'%str2latex(tableLogName+extraTexFileSuffix))
                return(outs)
            ...
            if skipStata==True: 
                ...
                * regTable: Skipping reestimation of entire table %s (logged in %s) because skipStata was set True """%(tablename,tablenamel)+self.skipStataForCompletedTables*' or because self.skipStataForCompletedTables was set True '+"""
    ...
    """
            ...
            allMethods=uniqueInOrder([ss for ss in [mm.get('method','') for mm in models] if ss])
            allLHS=uniqueInOrder([ss for ss in [mm.get('depvar','') for mm in models] if ss])
            allModels=uniqueInOrder([ss for ss in [mm.get('model','') for mm in models] if ss])
            comments+=' Regression methods: %s. Dependent vars: %s '%(allMethods,allLHS)
    ...
    #def displayRegTable(self,tablename,models,method=None,depvar=None,regoptions=None,variableOrder=None,showonlyvars=None,hidevars=None,forceShowVars=None,extralines=None,comments='',substitutions=None,options='',attributes=None,landscape=False,transposed=None,combineRows=None,suppressSE=False, produceOnly=None,extraTexFileSuffix=None,doPcorr=False,stopForErrors=True,crcoefsVars=None,skipStata=False,skipLaTeX=False,hidePSumTest=False,defaultModel=None,hideModelNumbers=False,assignSaveCoefficientsPrefix=None,hideModels=None,showModels=None,hideModelNames=False,renumberModels=True,showFailedRegressions=False,multirowLabels=False,betas=False,followupFcn=None,followupArgs=None,showCompDiff=None,returnModels=False,postPlotFcn=None,postPlotArgs=None,autoCreateVars=True,captureNoObservations=None): # Do a set of regressions; output results to .tex and .txt files 
        ...
        if (hideModels and isinstance(hideModels[0],dict)) or (showModels and isinstance(showModels[0],dict)):
            ...
            if hideModels:#isinstance(hideModels,list) and hideModels and isinstance(hideModels[0],int):
                assert isinstance(hideModels,list) and isinstance(hideModels[0],int)
                showModels=[nn+1 for nn in range(len(models)) if nn+1 not in hideModels]
            if showModels:#isinstance(showModels,list) and showModels and  isinstance(showModels[0],int):
                ...
                if max(showModels)-1>=len(models):
                    print 'CAUTION!!!!!!!!!! Your list of model indices includes non-existing models!!!!!!!!!!! Ignoring  them...',[iM for iM in showModels if iM-1>=len(models)]
                models=[models[iM-1]  for iM in showModels if iM-1<len(models)]


        if not models:
            print ' AWWWWWWWWWWWWWWWWWWWW! No models left!! Aborting this table!'
            self.append(' AWWWWWWWWWWWWWWWWWWWW! No models left!! Aborting this table!')
            return()
        assert models
        ...
        badR2=[fNaN,1,1.0,'noR2'] # I think this should just be fNaN in newmode.
        for mm in models:
            if 'isManualEntry' in model:
                continue
                ...
                for mmm in models:
                    ...
                    if 'estCovar' in mmm:
                        ...
                        if isinstance(mmm['estCovar'],dict):
                            ...
                            mmm['estCovar'].rename(columns=dict([[vfrom,vto] for vfrom in vng[1:]]),inplace=True)
                            mmm['estCovar'].rename(dict([[vfrom,vto] for vfrom in vng[1:]]),inplace=True)
        ...
        for model in models:
            if 'CRgroup' in model:
                hidevars+=[vv for vv in model['estcoefs'] if vv.startswith('ddd_')]
        ...
        if combineRows:
            ...
            if any(['compDiffBy' in mm for mm in  models]):
                print(' N.B.: combineRows is defined, so will need to revise some keys like compDiffBy...')
        ...
        for imodel in range(len(models))[::-1]:
            if 'compDiffBy' not in models[imodel] or not models[imodel]['compDiffBy']:
                continue
            ...
            V=frommodel['estCovar'] # 2013 Feb: this changed from dict to pandas df
            incvar=frommodel['compDiffBy']
            if combineRows:
                for tofrom in combineRows:
                    if incvar in tofrom[1:]:
                        print('Using %s instead of %s as comp diff income'%(tofrom[0],incvar))
                        incvar=tofrom[0]
                        continue
       ...
       """+str((incvar  in frommodel['model'] , incvar  in frommodel['estcoefs'] , incvar  in frommodel['estCovar'])))
            ...
            cdvars=list(set(frommodel['estcoefs'].keys())-(set([incvar]+['_cons', 'age', 'agesq100', 'age100sq','age100t3','age100t4','age100',])))
                        

            # 'compDiffVars'= list or string list of variables for which comp diffs should be calculated and/or displayed. Note that regTable has a table-level parameter "showCompDiff" which decides whether/how comp diffs should be shown.
            if 'compDiffHideVars' in cdModel:
                if isinstance(cdModel['compDiffHideVars'],str):
                    cdModel['compDiffHideVars']=cdModel['compDiffHideVars'].split(' ')
                cdvars=[cv for cv in cdvars if cv not in cdModel['compDiffHideVars']]

            if 'showCompDiffVars' in cdModel:
                if isinstance(cdModel['showCompDiffVars'],str):
                    cdModel['showCompDiffVars']=cdModel['showCompDiffVars'].split(' ')
                cdvars=[cv for cv in cdModel['estcoefs'].keys() if cv in cdModel['showCompDiffVars'] and not cv ==invar]
            for vv in cdvars: # In following, x is the variable of interest, y is income, s is sigma, b is coefficient
                bx,by=frommodel['estcoefs'][vv]['b'],frommodel['estcoefs'][incvar]['b']
                sx,sy=frommodel['estcoefs'][vv]['se'],frommodel['estcoefs'][incvar]['se']
                sxy=V[vv].ix[incvar]
                compdiff=bx/by
        ...
        for model in models:
            ...
            incsum,seincsum=seSum([model['estcoefs'][vv]['b']  for vv in model['estcoefs'] if vv in possibleIncomeVars],
        ...
        if 0 and 'r(p)' in [ss[0] for ss in sumStatsRows]:
            if incomeCoefs:
                sumStatsRows.insert([ipair for ipair in range(len(sumStatsRows)) if sumStatsRows[ipair][0]=='r(p)'][0],[r'$\sum I$']+chooseSFormat(summedIncomecoefs,lowCutoff=1.0e-3))
        ...
        if 0:
            ...
            if hidevars: # Careful; this feature hasn't been properly commensurated with showonlyvars:
                extralines=[el for el in extralines if el[0].strip() not in noshowKeys]
        ...
        if produceOnly==None or produceOnlyLower in [None,'onlyraw']:
            if not skipLaTeX:
                self.appendRegressionTable(models,tableFormat={'caption':tablecaption,'comments':comments,'hideVars':hidevars,'variableOrder':variableOrder,'hideModelNames':hideModelNames},
                         suppressSE=suppressSE,substitutions=substitutions, tableFilePath=defaults['paths']['tex']+tableTexName+'.tex',
                          transposed=transposed, sourceLogfile=tableLogName if not skipReadingResults else None)#landscape=landscape,rowModelNames=rowModelNames,hideVars=hidevars,extralines,modelTeXformat=modelTeXformat, tableCaption=tablename,tableComments=comments
        ...
        if 1:
            ...
            for sg in sumGroups:
                sgbyVar,sgbyStat,sgbyTextraline= modelResultsByVar(sumGroups[sg])
                meanModel={'estcoefs':{},'eststats':{},'texModelNum':r'$\langle$%s-%s$\rangle$'%(str(sumGroups[sg][0].get('modelNum','??!')-0),str(sumGroups[sg][-1].get('modelNum','??!')-0)),'modelNum':0,'name':sumGroups[sg][0].get('meanGroupName',r'\cpblMeanRowName '),'tableshading':'grey','textralines':{},'isMean':True}

                meanModel['meanGroupName']=meanModel['name'] # why was this needed?
                assert meanModel['name'] # May 2011.. just reading through some code and not sure whether above is safe.
                for vv in sgbyVar:
                    mu,se=seMean(sgbyVar[vv]['coefs'],sgbyVar[vv]['ses'])
                    if mu not in [None,fNaN]:
                        meanModel['estcoefs'][vv]={'b':mu,'se':se,}
                    meanModel['eststats']['N']=sum(sgbyStat['N'])#'$\\geq$%d'%min(sgbyStat['N'])
                ...
                for vv in sgbyTextraline:
                    if all([sgbyTextraline[vv][0]==tt for tt in sgbyTextraline[vv]]):#All addends have the same value for this flag:
        ...
        if (not skipLaTeX) and (sumGroups or combineColumns) and ((produceOnly==None and self.skipAllDerivedTables==False) or produceOnlyLower in [None,'withmeans','means']):# or produceOnly.lower() not in ['means','crc','onlyraw']:
            self.appendRegressionTable(models,tableFormat={'caption':tablecaption+'~(with~means)','comments':comments,'hideVars':hidevars,'variableOrder':variableOrder,'hideModelNames':hideModelNames},
            suppressSE=suppressSE,substitutions=substitutions, tableFilePath=defaults['paths']['tex']+bothOutfilename+'.tex', transposed=transposed, sourceLogfile=tableLogName)
        ...
        if (sumGroups or combineColumns) and ((produceOnly==None and self.skipAllDerivedTables==False) or produceOnlyLower in [None,'means','justmeans','onlymeans']):# or produceOnly.lower() not in ['means','crc','onlyraw']:
            ...
            if 1:
                onlyMeans=deepcopy([mm for mm in models if not any([any([mm in sumGroups[sg]]) for sg in sumGroups])])
                if 0: #WTF does the following not work?
                    onlyMeans=[mm.update({'tableshading':None}) for mm in deepcopy(onlyMeans) ]
                for mm in onlyMeans:
                    mm['tableshading']=None

                if any(onlyMeans):
                    if  not skipLaTeX:
                        self.appendRegressionTable(onlyMeans,tableFormat={'caption':tablecaption+'~(only~means)','comments':comments,'hideVars':hidevars,'variableOrder':variableOrder,'hideModelNames':hideModelNames},
                         suppressSE=suppressSE,substitutions=substitutions, tableFilePath=defaults['paths']['tex']+justaggFilename+'.tex', transposed=transposed, sourceLogfile=tableLogName)
                        debugprint( 'Wrote "just-means" tex file... [newmethod]')
                else:
        ...
        lCRgroups=uniqueInOrder([m['CRgroup']['id'] for m in models if 'CRgroup' in m]) 
        #It seems the following isn't working; but is it even useful?
        CRgroups=dict([[crg,[mm for mm in models if mm.get('CRgroup',{}).get('id',None)==crg]] for crg in lCRgroups])
        ...
        if 0:
            ...
            if CRgroups and aggCRgroups and any(aggCRgroups):##any([any(['CRgroup' in k for k in m.keys()]) for m in models]):
                """
                ...
                eNcol=[ff for ff in crsumStatsRows if ff[0] in ['e(N)','N']]##'e(N)' == ff[0]]
                if eNcol:
                    eNcol=eNcol[0]

        if CRgroups:
            print('About to cycle through these CR groups: %s'%str(CRgroups.keys()))
        from cpblUtilities import flattenList
        ...
        for CRg in CRgroups:
            ...
            iLastModel=[imm for imm,mmm in enumerate(models) if mmm is CRgroups[CRg][-1]][0]
            ...
            firstLast=[mm for mm in originalCRmodels if not mm.get('isMean',False)]
            ...
            if any([mm.get('isMean',False) for mm in CRgroups[CRg] ]):
                ...
                CRgroups[CRg]=[mm for mm in CRgroups[CRg] if mm.get('isMean',False) ]
            ...
            if crcoefsVars==None:
                crvars=flattenList([vv for vv in gmodels[-1]['estcoefs'] if 'ln' in vv and 'ncome' in vv])
                if crvars==[]:
                    ...
                    # Uhhhh. hm.
                    crvars=flattenList([vv for vv in gmodels[0]['estcoefs'] if 'ln' in vv and 'ncome' in vv])

            else:
                crvars=[vv for vv in crcoefsVars if vv in gmodels[-1]['estcoefs']]
            print '    %s: CR coefficient collection mode: Looking for CRs %s and variables %s.'%(CRg,str(CRs),str(crvars))
            assert crvars
            newCRmodel=deepcopy(gmodels[-1])
            newCRmodel.update({'isCRcoefs':True ,'texModelNum':r'$\langle$%s-%s$\rangle$'%(str(firstLast[0].get('modelNum','??!')-0),str(firstLast[-1].get('modelNum','??!')-0)) })# ,'modelNum':0})
            # Need to fiddle here with some flags??
            for kk in ['%s~f.e.'%CR for CR in CRs if CR]: # Get rid of CR~f.e. entries. ...
                #if kk in newCRmodel['textralines']:

                newCRmodel['textralines'][kk]=r'\YesMark'
            newCRmodel['eststats']['N']='$\\geq$%d'%(min([int(gg['eststats']['N']) for gg in CRgroups[CRg]]))
            ...
            for im, gm in enumerate(gmodels[:-1]):
                for crvar in crvars:
                    ...
                    if copyCR.lower()+'_'+crvar in gm['estcoefs']:
                        newCRmodel['estcoefs'][copyCR.lower()+'_'+crvar]=deepcopy(gm['estcoefs'][copyCR.lower()+'_'+crvar])
                ...
                #topCRfeLine=[nel for nel in nextralines if topCR+'~f.e.' in nel[0] and any([col.strip(' ~0') for col in nel[1:]])]
                    ...
                    for CR in CRs:
                        ...
                        if crcoefsVars == None: # Should this be a string? list of vars?
                            incomeColsI[CR]=[ff for ff in  range(len(crpairedRows)) if CR.lower()+'-log' in crpairedRows[ff][0][0]]
                        else:
                            incomeColsI[CR]=[ff for ff in  range(len(crpairedRows)) if any([CR.lower()+'-'+crcoefvar in crpairedRows[ff][0][0] for crcoefvar in crcoefsVars]) ]
                    ...
                    for CR in incomeColsI.keys():
                        incomeCols[CR]=[crpairedRows[ff] for ff in incomeColsI[CR]]
...
##                     print(incomeCols['DA'])
                    ...
                    if hideModelNames:
                        ...
                        crcolumnName.insert(insertAt-1,'CR~coefs') # -1 because there's no label in [0]?

                    #crcolumnNumbers.pop(td-1) # This is reset below
                    nrowModelNames.insert(insertAt-1,'')# huh??? what is this again? row or col
                    ...
                    for skipThisOldMethod in []:#ig in igroup[::-1]: # Loop backwards over models within our group so we go from small to large: ie algorithm still depends on the models being called in the right order. (?) ????
                        ...
                        if iii:
                            ...
                            for CRtoCopy in CRstoCopy:
                                ...
                                print('<CR>: So I am looking for rows with '+CRtoCopy+'\n')
                                for ipr in incomeColsI[CRtoCopy]:
                                    ...
                                    crKeepRows+=[ipr]
                                    prow=crpairedRows[ipr]
                                    if prow[0][ig+1]: # Only copy over the values if they are not blank. So, for missing regressions, the model is counted as included but actually skipped in filling in the CR coefs row.
                                        prow[0][insertAt]=prow[0][ig+1]
                                        prow[1][insertAt]=prow[1][ig+1]
                                    ...
                                    print('In CRgoup %s, Copied value "%s" from indep var: %s to new indep var with position %d'%(groupName,prow[0][ig+1],prow[0][0],insertAt))
                        else:
                    ...
                    if eNcol:
                        eNcol[insertAt]='$>$%d'%(min([int(eNcol[iii+1]) for iii in igroup if int(eNcol[iii+1])>0 ]))
        ...
        madeOutputCRC=False
        if (CRgroups) and not skipLaTeX  and [mm for mm in models if mm.get('isCRcoefs',False) ] and  ((produceOnly==None and self.skipAllDerivedTables==False) or produceOnlyLower in [None,'withcrc','crc']) :
            self.appendRegressionTable([mm for mm in models if not mm.get('hideInCRCview',False)],tableFormat={'caption':tablecaption+'~(with~crc)','comments':comments,'hideVars':hidevars,'variableOrder':variableOrder},
                                       suppressSE=suppressSE,substitutions=substitutions, tableFilePath=defaults['paths']['tex']+crcFilename+'.tex', transposed=transposed, sourceLogfile=tableLogName)
            print ('Wrote "with CRC coefs" tex file...')
            madeOutputCRC=True

        if (CRgroups) and ((produceOnly==None and self.skipAllDerivedTables==False) or produceOnlyLower in [None,'withcrc','onlycrc','crc']) and [mm for mm in models if mm.get('isCRcoefs',False) ] and not skipLaTeX:
            self.appendRegressionTable([mm for mm in models if mm.get('isCRcoefs',False) ],tableFormat={'caption':tablecaption+'~(only~crc)','comments':comments,'hideVars':hidevars,'variableOrder':variableOrder},
                                       suppressSE=suppressSE,substitutions=substitutions, tableFilePath=defaults['paths']['tex']+crcOnlyFilename+'.tex', transposed=transposed, sourceLogfile=tableLogName)
            print ('Wrote "only CRC coefs" tex file...')
            madeOutputCRC=True

        if not madeOutputCRC and defaults['mode'] in ['RDC']:# or 'redux' not in tablenamel or 'ontrol' not in extraTexFileSuffix
            print '   No CRC-containing tables were made for  %s (%s) because of produceOnly, no CRcoefs, or some other reason '%(tablenamel,extraTexFileSuffix)
        ...
        if followupFcn: # Maybe this call should come earlier, if models gets too fiddled with.
            standardArgs={'tableFileName':tableFileName,'tablename':tablename,'substitutions':substitutions}
    ...
    def includeFig(self,figname=None,caption=None,texwidth=None,title=None):#,,onlyPNG=False,rcparams=None
        ...
        \clearpage\newpage\clearpage
%\begin{landscape}
\cpblFigureTC{"""+figlocation+figfile+'.pdf}{'+texwidth[1:-1]+r'}{ '+title+'}{'+title+'}{'+ caption+''+'}{'+figfile+"""}
%\end{landscape} 
\clearpage
"""
...
\caption["""+figname + title+r'AAA]{ \ctDraftComment{'+figlocation+figfile+'}'+ title+''+caption+0*(r' \ctDraftComment{'+figlocation+figfile+'}')+r"""
\label{fig:"""+figfile+r"""} }
\end{figure} 
%\end{landscape} 
\clearpage
                """
        figtout=open(defaults['paths']['tex']+'tmpFig-'+figfile+'.tex','wt',encoding='utf-8')
...
June 2012: Added / Passing rv option through to savefigall
        ...
        if onlyPNG:
            ...
            if (skipIfExists or classSaysSkipUpdate) and os.path.exists(figlocation+figfile+'.png') and os.path.exists(figlocation+figfile+'.pdf'):
                print '   Skipping  saving figure '+figlocation+figfile+' because it already exists and self.skipStataForCompletedTable==True'
        ...
        if rootAndTail:
            self.includeFig(figlocation+figfile,caption=caption,texwidth=texwidth)
        ...
        * CPBL END TABLE:%s: AT %s
            ...
            * Closing %s
            capture noisily log close
             """%(tableName,time.strftime('%Y_%m_%d_%H%M%S'),tableLogNameWithDate)+"""
* Since we've successfully finished, set the preceeding log file as the conclusive one
 copy "%s" "%s", replace
 """%(tableLogNameWithDate,tableLogName)
        ...
        fa=re.findall("""\* CPBL BEGIN TABLE:%s:(.*?)\* CPBL END TABLE:%s:"""%(tableName,tableName),''.join(open(tableLogName,'rt').readlines()), re.DOTALL)
        assert len(fa)==1
        ...
        for iv,oneVar in enumerate(vps):
            ...
            pWs=latexFormatEstimateWithPvalue(pW,pval=pW,allowZeroSE=None,tstat=False,gray=False,convertStrings=True,threeSigDigs=None)
            body+=[[substitutedNames(v1,substitutions),xs1,xs2,pWs],
                   ['' ,ses1,ses2,''],
                   ]
...
So, this can be used either in a sort of automatic mode when closing a LaTeX file, e.g. based on the variables that have been used altogether, or it can be used to make an intermediate table of stats in the middle of a LaTeX file.
 ...
 I NEED EXAMPLES HERE.........  (no, see docs above (in latex class?))
        ...
        if dataFile and self.mainDataFile and not dataFile == self.mainDataFile:
            print 'CAUTION! Overriding %s with %s for main survey for stats'%(self.mainDataFile,dataFile)
        if not dataFile:
...
"""
        ...
        if not len(uniqueInOrder(showVars)) == len(showVars):
            print(" UHHHHH: You probably want to fix this so as not to screw up order...")
        ...
        if dataFile:
            ...
            if tablename and not 'pre-2013 mode': 
                ...
                logfname='summaryStatisticsFromStata_%s'%(ff.replace('.','_'))#%os.path.splitext(ff)[0]
                tablename=logfname
            print 'Generating automatic descriptive statistics from %s using weight/condition "%s", and variables "%s" and condition %s into logfile %s.'%(dataFile,str(weightsif),vString,str(ifcondition),logfname)

        else:
            print ' PRoducing STata code to generate descriptive statistics using weight/condition "%s", and variables "%s".'%(str(weightsif),vString)
...
N.B. As for 2010Feb, I am not yet reading/using the mean calculation.
        ...
        tablenamel=self.generateLongTableName(tablename,skipStata=False)#skipStata)
...
#            skipStata=True
        ...
        if execStata and not forceUpdate and not fileOlderThan(tableLogName,dataFile) and 'Succeeded' in open(tableLogName,'rt').read():
        ...
        if not execStata and not os.path.exists(tableLogName):
            print " ****  SKIPPING THE DESCRIPTIVE STATS IN %s BECAUSE YOU HAVEN'T RUN the regressions STATA YET to make it."%(tableLogName)
...
"""+ '\n'.join([r'\s+r.%s. =\s+(\S*)'%ff[0] for ff in sfields]) +r'.*?\n-~-~-~-~-~-~-~-~[^~]*\|[^~]*\|\s*([^\s]*)\s+([^\s]*)\s+([^\s]*)\s+([^\s]*)\n'  +\
        ...
        if 0: # 2012 and earlier versoin:
            fa=re.findall(  sstr,''.join(open(tableLogName,'rt').readlines()), re.DOTALL)
        # 2013 new version: for speed, split up the regexp task:
        #logtxt=''.join(open(tableLogName+'.log','rt').readlines())
        vsections=re.findall( '-=-=-=-=-=-=-=-=(.*?)x~x~x~x~x~x~x~x~x~',''.join(open(tableLogName,'rt').readlines()), re.DOTALL) # Fast
...
"""
        ...
        for vsection in vsections:
            ...
            mDescstats=zip([mf[1] for mf in mfields],  re.findall(meanStr2013,piecesA[0][-1],re.DOTALL)[0])

            sStr='\n'.join([r'\s+r.%s. =\s+(\S*)'%ff[0] for ff in sfields])

            sDescstats=zip([mf[1] for mf in sfields],  re.findall(sStr,piecesA[0][5])[0])
            ...
            ifClause=vv[1].strip() # New June 2010: The name of the if clause is available in the log file now. I suppose I could do an idiot check here to check for changes in the correspondence between ifs and ifnames (ie in caller vs log file). Not done.
        ...
        notFound=[vv for vv in showVars if vv not in [dsv['var'] for dsv in descStats2]]
        if notFound and execStata and not forceUpdate:
            if 'yes'==raw_input('  ---> Some variables (%s) not found in the summary statistics log file. Shall I delete so it will be recreated next time? ([no]/yes)'%str(uniqueInOrder(notFound))):
        ...
        if ifcondition==None:
            ifcondition=[]
        print "Got %d sumstats from %s for %d requested variables for %d conditions."%(len(descStats2[0].keys())-2,tableLogName,len(showVars),len(ifcondition))

        if ifNames and len(ifcondition)>1:
            assert ifOrder==ifNames
            print '  -------> I am going to create a CSV which combines a bunch of ifconditions for the descriptive stats! ('+str(ifNames)+') '
            allIfsCSV=''
        ...
        for iIf,anifcond in enumerate(ifOrder):#byIf.keys()):
            ...
            for vv in byIf[anifcond]:
                ...
                codebookT[vv]['sumstats']=byIf[anifcond][vv][0]#ds2#escStats2[vv]
            

            # Now generate a table with these data.
            comments='If some variables have missing values, try deleting %s.'%str2latex(tableLogNameNoSuffix) + caption
            if os.path.exists(tableLogNameNoSuffix+'.tex'):
                import time
                statsDate=time.ctime(os.path.getmtime(tableLogNameNoSuffix+'.tex'))
...
###            from cpblUtilities import cpblTableToCSV
        ...
        if mode=='compareMeans':
            ...
            comments='If some variables have missing values, try deleting %s.'%str2latex(tableLogName) + caption
            if os.path.exists(tableLogNameNoSuffix+'.tex'):
                import time
                statsDate=time.ctime(os.path.getmtime(tableLogNameNoSuffix+'.tex'))
                comments+=' Stats carried out on '+statsDate
            ...
            body=[]
            from dictTrees import dictTree
            statst=dictTree(tonumeric(descStats2),['var','if']).singletLeavesAsDicts()
            for vvv in showVars:
                tE=statst[vvv][ifNames[0]]
                xs1,ses1= latexFormatEstimateWithPvalue([tE['mean'],tE['se']],pval=tE['p'],allowZeroSE=None,tstat=False,gray=False,convertStrings=True,threeSigDigs=None)
                tE=statst[vvv][ifNames[1]]
                xs2,ses2= latexFormatEstimateWithPvalue([tE['mean'],tE['se']],pval=tE['p'],allowZeroSE=None,tstat=False,gray=False,convertStrings=True,threeSigDigs=None)
                body+=[[vvv,xs1,xs2,'?'],
                       ['' ,ses1,ses2,''],
                       ]
            cpblTableStyC(cpblTableElements(body='\\\\ \n'.join(['&'.join(LL) for LL in body])+'\\\\ \n\\cline{1-\\ctNtabCols}\n ',format=None,firstPageHeader=' & '.join(headers)+'\\\\ \n\\hline\\hline\n',otherPageHeader=None,tableTitle=None,caption=None,label=None,ncols=None,nrows=None,footer=None,tableName=None,landscape=None),
                          filepath=tableLogNameNoSuffix+'-compareMeans-'+str2pathname('-'.join(ifNames))+'.tex',masterLatexFile=self)
    ...
    def closeAndCompile(self,launch=True,closeOnly=False,compileOnly=False,showStatsFor=None,statsCondition=None,noStats=False): # ,dataFile=None
        """
...
What is "launch"? It seems not used.
        ...
        if not compileOnly and not noStats:
            if self.codebook:# and isinstance(self.codebook,stataCodebookClass):
                self.addDescriptiveStatistics(codebook=self.codebook,logname='end',showVars=showStatsFor,ifcondition=statsCondition)
            #elif self.codebook and isinstance(self.codebook,str):
    ...
    ###########################################################################################
    ###
    def subSampleAccounting(self,onemodel,dummyArgsNotUsed=None):#,followupArgs):
        ###
        #######################################################################################
...
2015: This is surely entire entirely obselete now. see oaxacaThreeWays
        ...
        model['getSubSampleSums']: a list of if def conditions that specify sub-samples
        model['subSampleNames']: a list of names for those sub-samples
        model['subSums']: a dict of results from Stata giving means for variablees in each sub-sample
        model['subSumPlotParams']: dict of further optional parameters
             model['subSumPlotParams']['groupVars']: dict naming lists of RHS variables to group together in the plot! e.g. age variables could be grouped.
             model['subSumPlotParams']['showOnly']: this affects the table output as well as plot: it says just show a subset of the subSample regions that were specified. This means that you can use original log files but change the tabular/plot output without throwing a compaint/warning, or rerunning Stata.
        

12 Jan 2010: oh-oh. Need to take variation in coefficients more seriously.

This can be called outside of regTable, if desired!

        """

        from cpblUtilities import categoryBarPlot
        from pylab import array,show
        import pylab as np

        if isinstance(onemodel,list):
            for mm in onemodel:
                if 'getSubSampleSums' in mm:
                    self.subSampleAccounting(mm)
            return()
        #for model in models:
        model=onemodel
        # Start with a sanity checks:
        if 'getSubSampleSums' in model:
            if not 'subSums' in model or  not len(model['getSubSampleSums'])==len(model['subSums']) or not all([anif.replace('cssaSample & ','').strip('()') in model['getSubSampleSums'] for anif in model['subSums']]):
                print(' subSampleAccounting: The subsample conditions in Stata output do not match those in the model description, so SKIPPING this model #%d (%s)'%(model['modelNum'],model['name']))



                return()
        else: # Did not request subSampleAccounting for this model
            return()
        assert model['method'] in ['svy:reg','svy:regress','reg','regress','xtreg']
        assert '*' not in model['model']

        if not 'subSampleNames' in model:
            model['subSampleNames']=model['getSubSampleSums']
        basecase=model['subSampleNames'][0]

        # So rename the subsamples to get rid of the cssaSample:
        #for anif in model['subSums'].keys():
        #    model['subSums'][anif.replace('cssaSample & ','')]=model['subSums'][anif]
        #    assert 'cssaSample & ' in anif #not anif.replace('cssaSample & ','') == anif:
        #    del model['subSums'][anif]

        # So rename the subsamples to get rid of the cssaSample:
        assert 0 < renameDictKey(model['subSums'], [[anif,anif.replace('cssaSample & ','').strip('()')] for anif in model['subSums']])



        subNames=model['subSampleNames']#getSubSampleSums']
        compNames=[sn for sn in subNames if not sn==basecase]
        # Use nicer names for the subgroups:
        renameDictKey(model['subSums'], zip(model['getSubSampleSums'],model['subSampleNames']))  


        # For convenience, add constant term to means: (and thus rhsvars) 
        for subsamp in subNames:
            model['subSums'][subsamp].update({'_cons':{'mean':1.0,'seMean':0.0}})

        assert '_cons' in model['estcoefs']


        # Before we rename variables, get some matrices
        assert model['estCovar']
        assert model['variableCorr']
        depvar=model['depvar']
        allvarstmp= [vv for vv in model['subSums'][basecase].keys()  if vv in  model['estcoefs'] or vv==depvar] 
        Sigmab=model['estCovar'] # Sigma b is the covariance matrix of coefficient estimates.
        Sigmax=model['variableCorr'] # n.b. this is a correlation matrix, not a variance-covariance matrix

        if 0:
            Sigmab=namedLookupToArray(model['estCovar'],[vv for vv in allvarstmp if vv not in ['constant',depvar]])
            assert 0



        # And use nicer (LaTeX?) names for the variables, if this is not too fragile:
        if 1:
            assert len(uniqueInOrder(substitutedNames(allvarstmp,model['substitutions'])))==len(allvarstmp)
            tmpVlookup=zip(allvarstmp,substitutedNames(allvarstmp,model['substitutions']))
            for subsamp in subNames:
                renameDictKey(model['subSums'][subsamp],tmpVlookup)
                renameDictKey(model['estcoefs'],tmpVlookup)
            for vvv in [vv for vv in allvarstmp if vv not in ['_cons','constant',depvar]]:
                renameDictKey(Sigmab[vvv],tmpVlookup)
                renameDictKey(Sigmax[vvv],tmpVlookup)
            renameDictKey(Sigmab,tmpVlookup)
            renameDictKey(Sigmax,tmpVlookup)
            depvar=substitutedNames(model['depvar'],model['substitutions'])
            if 'subSumPlotParams' in model and 'groupVars' in model['subSumPlotParams']:
                for groupName in model['subSumPlotParams']['groupVars']:
                    model['subSumPlotParams']['groupVars'][groupName]=substitutedNames(model['subSumPlotParams']['groupVars'][groupName],tmpVlookup)
             
            plotparams={'comments':''}#'hideVars':['constant','_cons']}
            #plotparams={'hideVars':'','showVars':model['estcoefs'].keys()}
            if 'subSumPlotParams' in model:
                plotparams.update(model['subSumPlotParams'])
            else:
                plotparams={}#'hideVars':'','showVars':model['estcoefs'].keys()}
            if 0 and 'hideVars' not in plotparams and 'showVars' not in plotparams:
                plotparams.update({'showVars':model['estcoefs'].keys()})
            """if 'subSumPlotParams' in model:
                plotparams=model['subSumPlotParams']
            else:
                plotparams={'hideVars':'','showVars':model['estcoefs'].keys()}
             """
            
            if 'hideVars' in plotparams:
                if isinstance(plotparams['hideVars'],str):
                    plotparams['hideVars']=plotparams['hideVars'].split(' ')
                plotparams['hideVars']=substitutedNames(plotparams['hideVars'],model['substitutions'])+['constant','_cons']
            if 'showVars' in plotparams:
                if isinstance(plotparams['showVars'],str):
                    plotparams['showVars']=plotparams['showVars'].split(' ')
                plotparams['showVars']=substitutedNames(plotparams['showVars'],model['substitutions'])



        
        predictions,sepredictions,diffpredictions,sediffpredictions,difflhs,sedifflhs,difffracs,rhsmeans,diffmeans,sediffmeans,sumDiffFracs,tooSmallToPlot={},{},{},{},{},{},{},{},{},{},{},{}
        for subsamp in subNames:
            predictions[subsamp],sepredictions[subsamp],diffpredictions[subsamp],sediffpredictions[subsamp],difflhs[subsamp],sedifflhs[subsamp],difffracs[subsamp],rhsmeans[subsamp],sumDiffFracs[subsamp],diffmeans[subsamp],sediffmeans[subsamp],tooSmallToPlot[subsamp]={},{},{},{},{},{},{},{},{},{},{},[]
        basecase=model['subSampleNames'][0]#['getSubSampleSums'][0]
        compcases=model['subSampleNames'][1:]



        # Variables that are not in estcoefs were dropped in the regression (or have a wildcard, asserted not above [ie wildcards not allowed when accounting called]), so ignore them.
        # rhsvars should be the same for every subsample:
        allvars= [vv for vv in model['subSums'][basecase].keys()  if vv in  model['estcoefs'] or vv==depvar] 
        rhsvars=[vv for vv in allvars if not vv == depvar and vv in  model['estcoefs']]
        from matplotlib import rc
        from pylab import sqrt,isnan
        from cpblUtilities import seProduct, seQuotient, seSum
        if not defaults['inRDC']:
            rc('text', usetex=True)


        for subsamp in subNames:
            for vv in allvars: # First, straightforward calculation of difference between mean values of each variable. No covariance cross-terms.
                diffmeans[subsamp][vv],sediffmeans[subsamp][vv]=seSum(x=[model['subSums'][subsamp][vv]['mean'],-model['subSums'][basecase][vv]['mean']],sx=[model['subSums'][subsamp][vv]['seMean'],-model['subSums'][basecase][vv]['seMean']])
            for vv in rhsvars:
                # Calculate predicted contributions and their standard errors. I don't need covariance matrix for this. ie No covariance cross-terms.
                ###model['subSums'][subsamp][vv]['semean']=model['subSums'][subsamp][vv]['stddev']/sqrt(model['subSums'][subsamp][vv]['N'])
                predictions[subsamp][vv],sepredictions[subsamp][vv] =  seProduct(x=model['subSums'][subsamp][vv]['mean'] , y=model['estcoefs'][vv]['b'], sx=model['subSums'][subsamp][vv]['seMean'] , sy=model['estcoefs'][vv]['se'])


            # But how to calculate s.e. for the predicted value? That takes full covariance matrix. Not yet done. Actually... it's not simple. Let's just use independent coefficient values for now:
            # Note that if I calculate predicted components for two regions and then difference them, I get the wrong s.e. because the two "b"s in the first calculation are correlated. So much better is to take a difference of the means form the two regions and then multiply that by the "b" coefficient once. Then the s.e. should be correct with no covariance terms.  However, to calculate the prediction for the dependent variable over some subset is much harder, requiring covariance between different independent variables AND between different coefficients.

            # This line says the prediction of the LHS is the sum of the contributions from all RHS variables: But IT IGNORES ALL THE COVARIANCE TERMS OF BOTH MEANS AND COEFFFICIENTS
            assert not any(isnan([sepredictions[subsamp][vv] for vv in rhsvars]))
            predictions[subsamp][depvar],sepredictions[subsamp][depvar]=seSum(x=[ predictions[subsamp][vv] for vv in rhsvars],sx=[ sepredictions[subsamp][vv] for vv in rhsvars])
        ...
        for subsamp in compNames:
            ...
            # So now just the following line has big problems: need covariances
            diffpredictions[subsamp][depvar],sediffpredictions[subsamp][depvar] = seSum(x=[predictions[subsamp][depvar],-predictions[basecase][depvar]],
                                                                                        sx=[sepredictions[subsamp][depvar],sepredictions[basecase][depvar]])

            # Do the hardest piece: s.e. for diff predicted: See the LyX file on standard errors
            rhsvarsnc=[vv for vv in rhsvars if vv not in ['_cons','constant']]
            sediffpredictions[subsamp][depvar]=sqrt(sum([sum([diffmeans[subsamp][vi]*diffmeans[subsamp][vj]*Sigmab[vi][vj] + 2*model['estcoefs'][vi]['b']*model['estcoefs'][vj]['b']*Sigmax[vi][vj]*model['subSums'][subsamp][vi]['seMean']*model['subSums'][subsamp][vj]['seMean'] for vi in rhsvarsnc]) for vj in rhsvarsnc]))

#            assert  abs(sediffpredictions[subsamp][depvar])<abs(diffpredictions[subsamp][depvar])



        # Do this also for any GROUPS of explanatory variables whose contributions we wish to pool:
        varsMovedToGroup=[]
        vgroupComments=''
        if 'groupVars' in plotparams:
            for vgroup in plotparams['groupVars']:
                ...
                for subsamp in compNames:
                    diffpredictions[subsamp][vgroup],sediffpredictions[subsamp][vgroup] = seSum(x=[predictions[subsamp][vgroup],-predictions[basecase][vgroup]],
                                                                                        sx=[sepredictions[subsamp][vgroup],sepredictions[basecase][vgroup]])
                    sediffpredictions[subsamp][vgroup]=sqrt(sum([sum([diffmeans[subsamp][vi]*diffmeans[subsamp][vj]*Sigmab[vi][vj] + 2*model['estcoefs'][vi]['b']*model['estcoefs'][vj]['b']*Sigmax[vi][vj]*model['subSums'][subsamp][vi]['seMean']*model['subSums'][subsamp][vj]['seMean'] for vi in vgroupvars]) for vj in vgroupvars]))
        ...
        tmpLcn=len(compNames)
        compNames=[compNames[iii] for iii in model.get('subSumPlotParams',{}).get('showOnly',range(len(compNames)))]
        if len(compNames)<tmpLcn:
            print '  Restricted comparison sets for display, down from %d to '%tmpLcn,str(compNames)


        for subsamp in compNames: # Loop over the comparison subsets (regions or etc), making an ACCOUNTING PLOT for each
            assert depvar in diffpredictions[subsamp]
            difffracs[subsamp]={}
            #difflhs[subsamp]=model['subSums'][subsamp][depvar]['mean'] - model['subSums'][basecase][depvar]['mean']
            #sedifflhs[subsamp]=sqrt( (model['subSums'][subsamp][depvar]['seMean'])**2.0 + (model['subSums'][basecase][depvar]['seMean'])**2)

            # Following addends are independent, so there're no covariance terms missing:
            difflhs[subsamp],sedifflhs[subsamp]=seSum(x=[model['subSums'][subsamp][depvar]['mean'], - model['subSums'][basecase][depvar]['mean']],
                                                      sx=[model['subSums'][subsamp][depvar]['seMean'], model['subSums'][basecase][depvar]['seMean'] ])

            for vv in rhsvars:
                difffracs[subsamp][vv] = diffpredictions[subsamp][vv] / difflhs[subsamp]

            # A check: we may be missing some variables that were wildcard specified dummies, etc: How close did we get?
            sumDiffFracs[subsamp]=sum([difffracs[subsamp][vv] for vv in rhsvars])
            ...
            cbph=categoryBarPlot(array([depvar,'predicted '+depvar]+plotvars),
        ...
        yerr=array( [sedifflhs[subsamp],sediffpredictions[subsamp][depvar]]+[sediffpredictions[subsamp][vv] for vv in plotvars])   ,barColour={depvar:defaults['colors']['darkgreen'],'predicted '+depvar:defaults['colors']['green']})
            #np.figlegend(yerr,['SS','ww'],'lower left')
            assert depvar in ['SWL','ladder','{\\em nation:}~ladder'] # depvar needs to be in the two lookup tables in following two lines:
            shortLHSname={'SWL':'SWL','ladder':'ladder','{\\em nation:}~ladder':'ladder'}[depvar]
            longLHSname={'SWL':'satisfaction with life (SWL)','ladder':'Cantril ladder','{\\em nation:}~ladder':'Cantril ladder'}[depvar]
            # Could put here translations
            
            xxx=np.legend(cbph['bars'][0:3],[r'$\Delta$'+shortLHSname+' observed',r'$\Delta$'+shortLHSname+' explained','explained contribution'],{True:'lower left',False:'lower right'}[abs(np.xlim()[0])>abs(np.xlim()[1])])
            xxx.get_frame().set_alpha(0.5)

            #np.setp(np.gca(),'yticks',[])
            # Could you epxlain the following if?? 
            if plotparams.get('showTitle',False)==True:
                np.title(model['name']+': '+subsamp+': differences from '+basecase)
                np.title("Accounting for %s's life satisfaction difference from %s"%(subsamp,basecase))
                title=''
                caption=''
            else:
                ...
                caption=title
            np.xlabel(r'$\Delta$ %s'%shortLHSname)
            #np.subtitle('Error bars show two standard error widths')

            np.xlabel('mean and explained difference in '+longLHSname)
            np.ylim(-1,len(plotvars)+3) # Give just one bar space on top and bottom.
            #np.ylim(array(np.ylim())+array([-1,1]))
        ...
        # Now, if we have created any accounting, show them in a new TABLE.
        if not 'accountingPlot' in model:
            return()
        ...
        # Now should use my table-writing codes to write/transpose a table after I throw something together here.

        # Jan 2010: Revised version. I now make two versions at once: "rows" will be formatted for variables as columns. "cellsT" will be formatted for variables as rows. I am doing this using array tranposition and slicing, which is nice.
        ...
        rows=[     ['',]+[r'\begin{sideways}\sltheadername{%s}\end{sideways}'%str2latex(vv) for vv in [depvar]+rhsvars],\
        ['$b$:','',]+[model['estcoefs'][vv]['b'] for vv in rhsvars],
        ]
        ...
        # row 0: region names
        # row 1: sample sizes (N)
        # row 2: column headers: b, xbar, etc.
        # row 3 and 4: depvar estimate and s.e.s
        # rows 5::2: indep var estimates and s.e.s
        cellsT[3::2,0]=[depvar]+deepcopy(rhsvars)
        cellsT[2,:]=['','$b$',r'$\bar{x}$',r'$\bar{x}b$']+flattenList([[r'$\bar{x}$',r'$\Delta\bar{x}$',r'$\Delta\bar{x}b$',r'share (\%)'] for subsamp in compNames])
        # Following line(s) eliminates extra "&"s in lines that have multicolumns
        cellsT[0,:]=[None for cc in cellsT[0,:]] # Fill in with None's, which will be dropped before joining.
        cellsT[1,:]=[None for cc in cellsT[1,:]] # Fill in with None's, which will be dropped before joining.
        cellsT[0,0:(3+len(compNames))]=['','',]+[r'\multicolumn{2}{|c|}{%s}'%basecase]+[ r'\multicolumn{4}{|c|}{%s}'%subsamp for subsamp in compNames]
        # Add in the sample sizes, N:
        cellsT[1,0:(3+len(compNames))]=['','',]+[r'\multicolumn{2}{|c|}{N=%d}'%model['subSums'][basecase][depvar]['N']]+[ r'\multicolumn{4}{|c|}{N=%d}'%model['subSums'][subsamp][depvar]['N'] for subsamp in compNames] #str(model['eststats']['N'])
        
        # Fill in b:
        cellsT[5::2,1]=[model['estcoefs'][vv]['b']  for vv in rhsvars]
        # Fill in estimates with ses:

        """

"""
        arows=\
            formatPairedRow([[r'$\bar{x}$: '+basecase,model['subSums'][basecase][depvar]['mean']]+[model['subSums'][basecase][vv]['mean'] for vv in rhsvars], 
        [r'',model['subSums'][basecase][depvar]['seMean']]+[model['subSums'][basecase][vv]['seMean'] for vv in rhsvars],] ,allowZeroSE=True)+[
        [r'$\bar{x}b$: '+basecase,predictions[basecase][depvar]]+[predictions[basecase][vv] for vv in rhsvars], ]
        rows+=arows

        rows[-1][-1]=chooseSFormat(rows[-1][-1])+'\\\\ \\hline' # Add separator to close this subsamp


        # Take arows and weave it into TRANSPOSED-format:
        cellsT[3::2,2]=arows[0][1:] # base country means
        cellsT[4::2,2]=arows[1][1:] # base country mean se
        cellsT[3::2,3]=arows[2][1:] # base country xb
        ...
        for subsamp in compNames:
            arows=formatPairedRow([
        [r'$\bar{x}$: '+subsamp,model['subSums'][subsamp][depvar]['mean']]+[model['subSums'][subsamp][vv]['mean'] for vv in rhsvars], 
        ['',model['subSums'][subsamp][depvar]['seMean']]+[model['subSums'][subsamp][vv]['seMean'] for vv in rhsvars], 
        ] ,allowZeroSE=True)
            arows+=formatPairedRow([
        [r'$\Delta\bar{x}$: '+subsamp,diffmeans[subsamp][depvar],]+[diffmeans[subsamp][vv] for vv in rhsvars],
        ['',sediffmeans[subsamp][depvar],]+[sediffmeans[subsamp][vv] for vv in rhsvars],
        ] ,allowZeroSE=True)

            arows+=formatPairedRow([
        [r'$\Delta\bar{x}b$: '+subsamp,diffpredictions[subsamp][depvar]]+[diffpredictions[subsamp][vv] for vv in rhsvars],

        ['',sediffpredictions[subsamp][depvar]]+[sediffpredictions[subsamp][vv] for vv in rhsvars]
        ] ,allowZeroSE=True)


            arows+=[
        [r'share (\%): '+subsamp,'',]+[chooseSFormat(100.0*difffracs[subsamp][vv]) for vv in rhsvars],
        ]
        ...
        rows=chooseSFormat(rows)

        # And now weave the above into the TRANSPOSED format
        ncn=len(compNames)
        ars=array(rows)
        for icn in range(len(compNames)):
            from numpy import transpose
            # x:
            cellsT[3::2,4+icn*4]=transpose(ars[5+(3*2+1)*icn,1:])   # x, xb, etc
            cellsT[4::2,4+icn*4]=transpose(ars[6+(3*2+1)*icn,1:])  # se for above
            # dx:
            cellsT[3::2,5+icn*4]=transpose(ars[7+(3*2+1)*icn,1:])   # x, xb, etc
            cellsT[4::2,5+icn*4]=transpose(ars[8+(3*2+1)*icn,1:])  # se for above
            # bdx:
            cellsT[3::2,6+icn*4]=transpose(ars[9+(3*2+1)*icn,1:])   # x, xb, etc
            cellsT[4::2,6+icn*4]=transpose(ars[10+(3*2+1)*icn,1:])  # se for above
            # share %:
            cellsT[3::2,7+icn*4]=transpose(ars[11+(3*2+1)*icn,1:])   # x, xb, etc
            

        tableTitle=r'Detailed decomposition of %s model %s'%(depvar,model['texModelNum'])
        comments=r'Regions compared with %s, based on coefficients from model ... \ctDraftComment{%s}. $\bar{x}$ represents the mean value of a variable; $\Delta\bar{x}$ is the difference in a region as compared with %s; $\Delta\bar{x}b$ is the contribution of this difference to the difference in %s; and share \%% is size of this contribution compared with the total predicted difference.'%(basecase,model['subSumPlotParams']['comments'],basecase,depvar)
        ...
        ###cpblTableStyC(''.join(texrows[1:]),format='rc'+'c'*max([len(row) for row in transRows]),filepath='%s%s_%02d%s_subsampleTable-transposed.tex'%(defaults['paths']['tex'],model['tableName'],model['modelNum'],model['name']),firstPageHeader=texrows[0],otherPageHeader=texrows[0],tableTitle=None,caption=None,label=None,ncols=None,nrows=None,footer=None,tableName=None,landscape=None,masterLatexFile=self)
        ...
        arow=['$b$:']
        for subsamp in compNames:
            arow+=[r'$\bar{x}$: '+subsamp,r'$\Delta\bar{x}$: '+subsamp,   r'$\Delta\bar{x}b$: '+subsamp,r'share (\%): '+subsamp]
        rows=[arow]

        #print 'make verticalTitles an option? in cpbltablestyc????? no...'
        for RHSvar in rhsvars:
            arow=[RHSvar,model['estcoefs'][RHSvar]['b']]
            arowse=['','']
            for subsamp in compNames:
                arow+=[model['subSums'][basecase][depvar]['mean']]
                arowse=[]
            
            #rows+=\
            #    formatPairedRow([[r'$\bar{x}$: '+basecase,model['subSums'][basecase][depvar]['mean']]+[model['subSums'][basecase][vv]['mean'] for vv in rhsvars], 
            # [r'',model['subSums'][basecase][depvar]['seMean']]+[model['subSums'][basecase][vv]['seMean'] for vv in rhsvars],] )+[
            # [r'$\bar{x}b$: '+basecase,predictions[basecase][depvar]]+[predictions[basecase][vv] for vv in rhsvars], ]
            # rows[-1][-1]=chooseSFormat(rows[-1][-1])+'\\\\ \\hline' # Add separator to close this subsamp
...
# 
# 
#             rows=[     ['',]+[r'\begin{sideways}\sltheadername{%s}\end{sideways}'%vv for vv in [depvar]+rhsvars],\
#             ['$b$:','',]+[model['estcoefs'][vv]['b'] for vv in rhsvars],
#             ]
#             rows[-1][-1]=chooseSFormat(rows[-1][-1])+'\\\\ \\hline' # Add separator to close this subsamp
# 
#             # rows+=[
#             #['',depvar,],#'Predicted '+depvar,], ]
#             rows+=\
#                 formatPairedRow([[r'$\bar{x}$: '+basecase,model['subSums'][basecase][depvar]['mean']]+[model['subSums'][basecase][vv]['mean'] for vv in rhsvars], 
#             [r'',model['subSums'][basecase][depvar]['seMean']]+[model['subSums'][basecase][vv]['seMean'] for vv in rhsvars],] )+[
#             [r'$\bar{x}b$: '+basecase,predictions[basecase][depvar]]+[predictions[basecase][vv] for vv in rhsvars], ]
...
#             print '**** Still need to make a 2-sig-dig mode for chooseSFormat'
        ...
        for ic in range(0,len(cellsT[-1])): # Get rid of \hline and \\ in final row-pair, in all but (no, including.) last column
            ...
            if isinstance(cellsT[-2][ic],str):
                cellsT[-2][ic]=cellsT[-2][ic].replace(r'\hline','').replace(r'\\','')
            
        rows=chooseSFormat([[cc for cc in rr if not cc==None] for rr in cellsT])

        # And put some hlines back in:
        nCols=max([len(row) for row in rows])
        rows[0][0]=r'\cline{2-%d} '%nCols +rows[0][0] # hline at top
        rows[2][-1]+=r'\\ \cline{2-%d} '%nCols # hline after header.
        rows[4][-1]+=r'\\ \cline{2-%d} '%nCols # hline after LHS (dependent) variable
        
        ###rows[1][-1]+='\\\\ \\hline'
        texrows=[' & '.join([rr for rr in row if not rr==None])+'\\\\'*(r'\\' not in row[-1])+' \n' for row in rows]
        cpblTableStyC(cpblTableElements(body=''.join(texrows[3:]),cformat='r|c|cc|'+''.join(['cccc|' for ss in compNames]),firstPageHeader=texrows[0:3],tableTitle=tableTitle,caption=comments,label=None,ncols=None,nrows=None,footer=None,tableName=None,landscape=None,),
                      filepath='%s%s-%02d%s-subsampleTable-transposed.tex'%(defaults['paths']['tex'],model['tableName'],model['modelNum'],model['name']), masterLatexFile=self)  # 'c'*max([len(row) for row in rows])
    ...
    ###########################################################################################
    ###
    def addCorrelationTable(self,tablename,corrvars,ifClause=None,threeSigDigs=False,showSignificance=True,variableOrder=None,comments=None):
        ###
        #######################################################################################
        """
        2010 Nov. 
...
Jan 2011: Huh? But there is no "N" recorded in the log file for each correlation...
        ...
        tableLogName=defaults['paths']['stata']['tex']+str2pathname(tablenamel)+'.log' # Actually, a TSV file
        tableFilePath=defaults['paths']['stata']['tex']+str2pathname(tablenamel) # for .tex , below

        assert ifClause is None
        
        if os.path.exists(tableLogName):
            #cells=tsvToDict(tableLogName)
            cells=[LL.strip('\n').split('\t') for LL in open(tableLogName,'rt').readlines()]
            assert cells[0][-1]=='' # Bug in Stata's mkcorr? extra column. arghhhh
            header=cells[0][:-1]
            ...
            for iv,vva in enumerate(varsOrder):
                for jv,vvb in enumerate(varsOrder[:1+iv]): # Skip upper right hand triangle 
                    ...
                    if p.startswith('('):
                        if p=='(0.00000)':
                            ...
                            fp=float(p[1:-1])
                            p= chooseSFormat(p[1:-1],convertStrings=True,threeSigDigs=threeSigDigs,conditionalWrapper=[r'\coefp{','}'])#,lowCutoff=None,lowCutoffOOM=True,convertStrings=False,highCutoff=1e6,noTeX=False,threeSigDigs=False,se=None):
                    else:
                        assert p in ['']
                    ...
                    #    r=''
                    r= chooseSFormat(r,convertStrings=True,threeSigDigs=threeSigDigs)
                    if p and showSignificance:
                        significanceString=([' ']+[tt[0] for tt in significanceTable if fp<= tt[2]/100.0])[-1]
                        r=significanceString+r+'}'*(not not significanceString)
            ...
            for iv,vv in enumerate(varsOrder[:-1]):
                assert '#' not in vv
                tworows=formatPairedRow(  [[r'\sltrheadername{%s}'%str2latex(substitutedNames(vv,self.substitutions))]+[pcorrs[vv][vvb]['b']*(ivvb>=iv) for ivvb,vvb in enumerate(varsOrder[1:])],
                                        ['']+[pcorrs[vv][vvb]['p']*(ivvb>=iv) for ivvb,vvb in enumerate(varsOrder[1:])]])

                body+= '\t& '.join([cc for cc in tworows[0]])+'\\\\ \n'+r'\showSEs{'+\
                                '\t& '.join([cc for cc in tworows[1]]) +' \\\\ }{}\n'
            body+=r'\hline '+'\n' # Separate the coefs from extralines..
            headersLine='\t&'.join(['']+[r'\begin{sideways}\sltcheadername{%s}\end{sideways}'%substitutedNames(vv,self.substitutions) for vv in varsOrder[1:]])+'\\\\ \n'+r'\hline'
            comments=[comments,''][int(comments is None)]
            includeTeX,callerTeX=cpblTableStyC(cpblTableElements(body=body,cformat='c'*(len(varsOrder)+2),firstPageHeader=r'\ctSubsequentHeaders \hline ',otherPageHeader=headersLine,tableTitle=None,caption=r'[Correlations among key variables]{Correlations among key variables. '+comments+'}',label='tab:Correlations',ncols=None,nrows=None,footer=None,tableName=tablename,landscape=None),
    ...
    ###########################################################################################
    ###
    def writeMultiSurveyCodebookTable(self,surveys,vars=None, findAllCommonVars=False,tablename='tmpxxx',maxVars=1e6):
        ###
        #######################################################################################
        """
        ...
        vvk.sort(reverse=True,key=lambda x:  'rawname' in byVV[x].values()[0][0])
        ...
        cpblTableStyC(cpblTableElements(body='\n'.join([str2latex(avv.replace('_','-'))+'\t& '+  '\t& '.join([r'$\checkmark$'*(asurvey in byVV[avv]) for asurvey in surveys])+r'\\'  for avv in vvk]),cformat=None,firstPageHeader=' & '.join(['']+surveys)+r'\\',otherPageHeader=None,tableTitle=None,caption=None,label=None,ncols=len(surveys)+1,nrows=len(vvk),footer=None,tableName=tablename,landscape=None),
                      filepath=paths['tex']+str2pathname(tablename)+'.tex', masterLatexFile=self)
...
"""%{'rx':rollingX}
        ...
        ifs=[' %s >= %f & %s < %f '%(ordinalRollingX,leftlim[iL],ordinalRollingX,leftlim[iL+1] ) for iL in range(len(leftlim)-1) ]

        print 'There are %d ifs for ns=%d'%(len(ifs),nSegments)
        for yrolling in allRollingCovariates+includeConstant*['qconstant']: # Prepare each of the rolling independent variables by making piecewise versions of them.
            ...
            * Let's recreate these
            ...
            for iL in range(len(leftlim)-1):
                outModel+="""
                gen """+yrolling+'_q%02d%02d'%(iL,len(leftlim)-1)+' = '+yrolling+' * ('+ifs[iL] +""")
                """
                covRollingNames[yrolling].append(yrolling+'_q%02d%02d'%(iL,len(leftlim)-1))
        ...
        for mmm in lmodels:
            if rollingCovariates is None: # I will probably just make it so flags always get set, above.
                mmm['model']= ' '+' '.join(flattenList([covRollingNames[vvv] for vvv in dgetget(mmm,['flags','rollingCovariates'],' ').split(' ')+includeConstant*['qconstant']])) + ' '+ mmm['model']
            else:
        ...
        """%(rollingX,(not rollingX == ordinalRollingX)*(' (%s)'%ordinalRollingX),nSegments))
        ...
        for ifigure,mm in enumerate(lmodels):
            ...
            mm['x'+rollingX]=array([  mm['subSums'][anif][rollingX]['mean']  for anif in ifs])
            mm['xstep'+rollingX]=array([xx for xx in flatten([leftlim[0]]+ [[xx,xx] for xx in leftlim[1:-1]]+[leftlim[-1]] )])

            mm['xstep'+rollingX]=array([ii for ii in flatten([  [mm['subSums'][anif][rollingX]['min'], mm['subSums'][anif][rollingX]['max']]   for anif in ifs])])

            #mm['xstep'+rollingX]=array(flattenList([leftlim[0]]+ [[xx,xx] for xx in leftlim[1:-1]] , leftlim[-1]]))
            mm['sex'+rollingX]=array([  mm['subSums'][anif][rollingX]['seMean']  for anif in ifs])
            ...
            for icv,cvv in enumerate([vvv for vvv in allRollingCovariates+includeConstant*['qconstant'] if vvv not in suppressFromPlot and any([vv for vv in mm['estcoefs'].keys() if vv.startswith(vvv)])]):
                ...
                if 0:
                    mm['b'+cvv]=array([  mm['estcoefs'][vv]['b']  for vv in cvvs])
                    mm['bstep'+cvv]=array(flattenList([[mm['estcoefs'][vv]['b'],mm['estcoefs'][vv]['b']] for vv in cvvs]))
                    mm['sebstep'+cvv]=array(flattenList([[mm['estcoefs'][vv]['se'],mm['estcoefs'][vv]['se']] for vv in cvvs]))
                    mm['seb'+cvv]=array([  mm['estcoefs'][vv]['se']  for vv in cvvs])
...
#                plotWithEnvelope(mm['x'+rollingX], mm['b'+cvv],mm['b'+cvv]-sef*mm['seb'+cvv],mm['b'+cvv]+sef*mm['seb'+cvv],linestyle='-',linecolor=rollCovColourLookup[cvv],facecolor=rollCovColourLookup[cvv],lineLabel=cvvn,     laxSkipNaNsXY=True)
            ...
            plt.ylabel('Raw coefficients (for %s)'%substitutedNames(mm['depvar']))
            ...
            comments=r'$b_{\log(GDP/cap)}=%.02f\pm%.02f$'%(mm['estcoefs']['lnGDPpc']['b'],mm['estcoefs']['lnGDPpc']['se']) if 'lnGDPpc' in mm['estcoefs'] else ''
            comments+={1.96:r'95\% c.i.'}[sef]
            transLegend(comments=comments)#
            pltcomments='Coefficients in estimate of '+substitutedNames(mm['depvar'])+' (with '+';'.join(mm.get('flags',{}).keys()).replace(' ','')+')'+mm['texModelNum']+mm['tableName'][:4]
            self.saveAndIncludeFig(figname=('TMP%s-%02d'%(tablename,ifigure)+'-'.join([rollingX,mm['name']]+mm.get('flags',{}).keys())).replace(' ',''),caption=pltcomments)
        return({'statatext':outStata,'models':lmodels,'figures':figs})


    ###########################################################################################
    ###
    def coefficientsOnIndicators(self,statamodel=None,indicators=None, rollingXvar=None,tablename=None,variableOrder=None,rollCovColourLookup=None,includeConstant=True,nSegments=None):
        ###
        #######################################################################################
        """
    ...
    def plotAgeCoefficients(self,models):
        ...
        for mm in models:
            if 'estcoefs' not in mm:
                continue
            if 'beta' in mm['regoptions'] or dgetget(mm,['textralines','$\\beta$ coefs'],'') in ['\\YesMark']:
                continue
            ageCoefs=[dgetget(mm,['estcoefs',cc,'b'],0) for cc in 'age100','agesq100','agecu100','agefo100']
            age=arange(0,100)
            plot(age,sum([ageCoefs[ii]*(age/100.0)**ii for ii in range(4)]),hold=True,label=mm['texModelNum']+': '+str2latex(mm['model']))
        transLegend()
        return(gcf())
...
"""+ latex.addCorrelationTable('testMkCorr','gwp_beta*',ifClause=None)+"""
